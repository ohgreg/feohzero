...
int load_fen(Board *board, char *fen) {
  *board = (Board){0};
  ...
  while (fen[last] == '-')
    last++;
  if (fen[last] && fen[last + 1] && 'a' <= fen[last] && fen[last] <= 'h' &&
      '1' <= fen[last + 1] && fen[last + 1] <= '8') {
    file = fen[last] - 'a';
    rank = fen[last + 1] - '1';
  ...
  for (int piece = PAWN; piece <= KING; piece++) {
    ...
    while (pieces) {
      ...
      board->key ^= get_zobrist_piece(WHITE, piece, sq);
    }
    pieces = board->pieces[BLACK][piece];
    while (pieces) {
      int sq = pop_lsb(&pieces);
      board->key ^= get_zobrist_piece(BLACK, piece, sq);
    }
  }
  board->key ^= get_zobrist_castling(((int)board->castle_white << 2) |
                                     (int)board->castle_black);
  board->key ^= get_zobrist_enpassant(board->ep_square);
  if (board->turn == BLACK) {
    board->key ^= get_zobrist_side();
  }
...
void apply_move(Board *board, Move *move) {
  ...
  if (piece == KING && (board->castle_black != CANNOT_CASTLE ||
                        board->castle_white != CANNOT_CASTLE)) {
    if (turn == 0)
      board->castle_white = CANNOT_CASTLE;
    else
      board->castle_black = CANNOT_CASTLE;
  } else if (piece == ROOK && (board->castle_black != CANNOT_CASTLE ||
                               board->castle_white != CANNOT_CASTLE)) {
    if (turn == 0) { // white
      if (move->from == 0)
        board->castle_white &= ~CAN_CASTLE_OOO;
