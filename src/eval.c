...
int eg_king_table[64] = {-74, -35, -18, -18, -11, 15, 4, -17, -12, 17, 14, 17, 17, 38, 23, 11, 10,  17,  23, 15, 20, 45, 44, 13,  -8,  22,  24,  27,  26,  33,  26,  3,
                         -18, -4,  21,  24,  27,  23, 9, -11, -19, -3, 11, 21, 23, 16, 7,  -9, -27, -11, 4,  13, 14, 4,  -5, -17, -53, -34, -21, -11, -28, -14, -24, -43};

int *mg_pesto_table[6] = {mg_pawn_table, mg_knight_table, mg_bishop_table, mg_rook_table, mg_queen_table, mg_king_table};

int *eg_pesto_table[6] = {eg_pawn_table, eg_knight_table, eg_bishop_table, eg_rook_table, eg_queen_table, eg_king_table};
...
void init_tables(void) {
    ...
    for (int sq = 0; sq < 64; sq++) {
        for (piece = PAWN; piece <= KING; piece++) {
            mg_table_black[piece][sq] = mg_value[piece] + mg_pesto_table[piece][sq];
            eg_table_black[piece][sq] = eg_value[piece] + eg_pesto_table[piece][sq];
            mg_table_white[piece][sq] = mg_value[piece] + mg_pesto_table[piece][sq ^ 56];
            eg_table_white[piece][sq] = eg_value[piece] + eg_pesto_table[piece][sq ^ 56];
        }
    }
}
